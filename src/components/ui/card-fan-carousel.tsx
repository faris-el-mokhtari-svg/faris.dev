...
const OUTER_ROTATION_RAD = (21 * Math.PI) / 180;
const OUTER_SCALE = 0.7756;
const MAX_OFFSET_REM = 30;

/**
 * The x-offsets above are fixed rem values, so on a narrow container the
 * outermost card can reach past the edge. This returns the largest multiplier
 * that still keeps that card fully inside, measured from the live layout.
 */
function getFitMultiplier(container: HTMLElement, card: HTMLElement) {
  const cardWidth = card.offsetWidth;
  const cardHeight = card.offsetHeight;
  if (!cardWidth || !cardHeight || !container.clientWidth) return Infinity;

  // Bounding width of the outermost card once rotated and scaled down
  const halfSpan =
    ((cardWidth * Math.cos(OUTER_ROTATION_RAD) +
      cardHeight * Math.sin(OUTER_ROTATION_RAD)) *
      OUTER_SCALE) /
    2;

  const rootFontSize =
    parseFloat(getComputedStyle(document.documentElement).fontSize) || 16;
  const available = container.clientWidth / 2 - halfSpan;
  return Math.max(0, available / (MAX_OFFSET_REM * rootFontSize));
}

function getFanMultiplier(container: HTMLElement, card: HTMLElement | undefined) {
  const design = getResponsiveMultiplier(window.innerWidth);
  if (!card) return design;
  return Math.min(design, getFitMultiplier(container, card));
}
...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  useEffect(() => {
    ...
    const multiplier = getFanMultiplier(container, cardElements[0]);
    ...
    const updateHoverLayout = (hoveredSlot: number | null) => {
      const mult = getFanMultiplier(container, cardElements[0]);
